...
class ChinaipoNewsSpider(RedisSpider):
    name = "chinaiponews"
    ...
    def parse(self, response):
        ...
        for item in response.xpath('//ul[@class="htnews-ul"]/li'):
            news = {}
            news['thumb_img'] = item.xpath(
                'div/a/img/@data-original').extract_first()
            ...
            if not self.first_time:
                time = ''.join([t.strip() for t in item.xpath(
                    'p/span/text()').extract_first().split('·') if t.strip()])
        ...
        if next_page:
            page += 1
            yield Request('http://m.chinaipo.com/index.php?app=3g&mod=Index&act=catList&type=1&cid=100000&p={0}'.format(page), dont_filter=True, callback=self.parse, meta={'page': page})

    def parse_news(self, response):
        news = response.meta['news']
        body = response.xpath(
            '//div[@class="artical-summary"]').extract_first()
        if body:
            soup = BeautifulSoup(body)
            soup.span.decompose()
            news['description'] = soup.get_text(strip=True)

            content_eml = response.xpath('//div[@class="airtical-content"]').extract_first(
            ).replace('\n', '').replace('\r', '').replace('\t', '').strip()
            soup = BeautifulSoup(content_eml)
            news['publish_time'] = [x.strip(
            ) for x in soup.div.contents[0].contents[1].string.split('·') if x.strip()][0]
            ...
            for child in soup.div.contents[1:]:
                ...
                if t == None:
                    ...
                    if not is_deleted:
                        [news['companies'].append({'short_name': x.string, 'stock_id': re.search(r'\d+', x['href']).group()})
                         for x in tc if x.name == 'a' and x.string and x.string.find('新三板') == -1 and re.search(r'\d+', x['href'])]
                        t = ''.join([x.strip()
                                     for x in child.stripped_strings if x])
                else:
